    ...
    if not node.tsNodeIsNull():
      ...
      case nodeType:
        ...
        of "enum_specifier":
          ...
          if gStateRT.logUnhandled:
            echo fmt"[toast] nodeType unhandled: {nodeType} at {file}:{line}:{col}"
          discard
    else:
      return
...
proc printNim*(fullpath: string, root: TSNode): string =
  # TODO: add generation date/time
  result.add &"# generated by nimterop.ast.printNim for {fullpath} \n"
  result.add "{.experimental: \"codeReordering\".}\n"
